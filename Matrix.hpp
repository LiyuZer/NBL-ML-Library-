...
template <typename T>  class Matrix{
 ...
 public:
    ...
    Matrix(int array[], int sizeOfArray, Network<Node>* n, bool isDynamic){
        double size=1;
        int count=0;
        string s_temporary="";
        bool found=false;
    ...
    Matrix(int array[], T, int sizeOfArray, Network<Node>* n, bool isDynamic){
       ...
       Matrix(string s, Network<Node>* n, bool isDynamic);
    ...
    template <typename U> Matrix<U>& linearTransform(Matrix<U>& m);
   ...
   void connectNode(Node& forward); // I added U for this special case where a matrix of value ndoes can be connected to collective nodes
    ...
    void randomize(){
        for(int i=0; i<headers.size(); i++){
            T* n= new T(uniformRand(0, 1, 10000), nullptr, this->getHeaders()->at(0).getNetworkPointer(), true);
            headers.at(i)=*n;
        }
    }
 ...
 private:
    ...
    bool dynamic;
    MatrixCleaner<Matrix<T>>* cleaner;
...
template <typename T> Matrix<T>:: Matrix (string s, Network<Node>* n,  bool isDynamic){
   double size=1;
   int count=0;
   string s_temporary="";
   bool found=false;
...
template <typename T>  Matrix<value_node>& Matrix<T>:: addBias( parameter_node& m){
    ...
    Matrix* n= new Matrix(array,1, m.getNetworkPointer(), true);
    
    int size1=this->getHeaders()->size();
    Network<Node>* main=m.getNetworkPointer();
    for(int i=0; i<size1 ; i++){
        int arr[1]={i};
        value_node* v= new value_node( main, true);
...
template <typename T> template <typename U> Matrix<U>& Matrix<T>:: linearTransform(Matrix<U>& m){
    int array[1]={this->getNumbers()->at(0)};
    Matrix<U>* returnMatrix=new Matrix<U>(array,1, m.getHeaders()->at(0).getNetworkPointer(), true);
    vector<thread> v;
...
template <typename T> Matrix<value_node>& Matrix<T>:: attach_collective_nodes( int indexFunc){
    ...
    Matrix<T>* matrix= new Matrix<T>(array,1, this->getHeaders()->at(0).getNetworkPointer(), true);
    int size1=headers.size();
    for(int i=0; i<size1; i++){
        int array[1]={i};
        collection_node* c= new collection_node(1, this->getHeaders()->at(0).getNetworkPointer(), true);
...
template <typename T> Matrix<T>& Matrix<T>:: convBranch(Matrix& forward, int dimension, int channel){
    ...
    Matrix<T>* ptr=new Matrix(array,this->getHeaders()->at(0).getNetworkPointer(), true);
...
template <typename T> Matrix<T>& Matrix<T>:: operator^(Matrix& right){
    ...
    collection_node* c= new collection_node(0, true);
    value_node* v= new value_node(this->getHeaders()->at(0).getNetworkPointer(), true);
    *c>>*v;
    for(int i=0; i<headers.size(); i++){
...
template <typename T> void Matrix<T>:: update(bool timeForUpdate){
   
    for(int i=0; i<headers.size(); i++){
        headers.at(i).update(timeForUpdate);
    }
}
...
template <typename T> template <typename U> T& Matrix<T> :: correlateMultiplication( int startingRow,  int startingColoumn, int channel, Matrix<U>& kernel, int chan, Matrix<U>& bias){
    ...
    *c>>*returnSum;
    c->activate();
    collection_node* activationFunc= new collection_node(1 ,this->getHeaders()->at(0).getNetworkPointer(), true);
    int biasArray[3]={0,0,chan};

    (*returnSum+bias.getA(biasArray, 3))>>=*activationFunc;
    T* finalSum= new value_node(this->getHeaders()->at(0).getNetworkPointer(), true);
...
template <typename T> template <typename U>  Matrix<T>* Matrix<T>:: convolution(Matrix<U>& kernel, Matrix<U>& bias){
    ...
    Matrix<T>* v= new Matrix( array, 3, this->getHeaders()->at(0).getNetworkPointer(), true);
    vector<thread> threads;
    int numberThreads=0;
...
template <typename T> double test2(Network<Node>* main){
   collection_node* c= new collection_node(0,main, true);
    no<T>(main,c);
    value_node v(main,false);
    *c>>v;
